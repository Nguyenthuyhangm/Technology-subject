...
async function main(): Promise<void> {
        ...
        printJsonAndExit(
            {
                error: 'MISSING_URL',
                message:
                    'Usage: npx tsx src/main/refresh-price-hasaki.ts "<HASAKI_PRODUCT_URL>"',
            },
            1
        );
    }
    ...
    try {
        ...
        await page.waitForSelector('h1, .txt_name, .product-name', {
            timeout: 10000,
        }).catch(() => undefined);
        ...
        const payload: PriceSnapshotOutput = {
            ...
            inStock: Boolean(detail.inStock ?? detail.price),
