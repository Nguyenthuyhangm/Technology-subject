...
test.describe("Alert - Full flow test", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/", { waitUntil: "domcontentloaded" }).catch(() => {});
  ...
  /**
   * Test 05: Full flow - Search, set alert, verify, delete
   * Combines all steps into one comprehensive test
   */
  // test("05 — Full alert flow: search, set, verify, delete", async ({ page }) => {
  //   // ===== STEP 1: Search và đặt alert =====
  //   await page.goto("/search?q=kem", { waitUntil: "domcontentloaded" });
  ...
  //   // Đặt alert với giá mục tiêu 50000
  //   await fillAlertFormAndCreate(page, "50000");

  //   // ===== STEP 2: Chuyển sang trang alerts =====
  //   await page.goto("/alerts", { waitUntil: "domcontentloaded" });
  //   await page.waitForTimeout(10000);
  ...
  //   // Verify sản phẩm vừa thêm có trong danh sách
  //   const isProductInList = await isProductInAlerts(page, productName);
  //   expect(isProductInList).toBe(true);

  //   // ===== STEP 3: Xóa sản phẩm khỏi alert =====
  //   await deleteAlertByProductName(page, productName);

  //   // Verify sản phẩm đã được xóa
  //   const isProductRemoved = await isProductInAlerts(page, productName);
  //   expect(isProductRemoved).toBe(false);

  //   // Verify danh sách trống hoặc không còn sản phẩm này
  //   const alertCount = await page.locator(SELECTORS.alerts.productCard).count();
  //   const isEmpty = await page.locator(SELECTORS.alerts.emptyState).isVisible({ timeout: 3000 }).catch(() => false);
  //   expect(isEmpty || alertCount === 0).toBe(true);
  // });
});
