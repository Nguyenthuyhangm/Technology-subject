...
export default function ProductSummary({ comparison, onAlertClick }: ProductSummaryProps) {
    ...
    const buyUrl = toAffiliateLink(bestOffer.url, bestOffer.platformName);

    return (
        <div className="rounded-[34px] border border-stone-200/80 dark:border-stone-700/40 bg-white dark:bg-[#1A1614] p-7 shadow-soft" style={{ fontFamily: FONT_STACK.sans }}>
            <h1 className="text-[2.45rem] leading-[1.05] tracking-[-0.02em] text-stone-900 dark:text-stone-100 md:text-[2.95rem]" style={{ fontFamily: FONT_STACK.serif }}>
                {comparison.productName}
            </h1>

            <div className="mt-5 flex flex-wrap items-center gap-3 text-sm text-stone-500 dark:text-stone-400">
                {bestOffer.isFlashSale && <Badge variant="danger">Flash Sale</Badge>}
                {bestOffer.inStock ? <Badge variant="success">Còn hàng</Badge> : <Badge variant="soft">Hết hàng</Badge>}
            </div>

            <div className="mt-8 rounded-[30px] bg-gradient-to-br from-[#F8F1F3] dark:from-[#2A1A1D]/60 to-white dark:to-stone-800/30 p-6">
                <p className="text-[11px] uppercase tracking-[0.12em] text-stone-400">Giá đẹp nhất hiện tại</p>
                <div className="mt-3 flex flex-wrap items-end gap-3">
                    ...
                    {bestOffer.originalPrice > bestOffer.price && (
                        <span className="pb-2 text-sm text-stone-300 line-through">{formatPrice(bestOffer.originalPrice)}</span>
                    )}
                    {bestOffer.discountPct > 0 && (
                        <span className="pb-2 text-sm font-medium text-[#8E6A72]">-{bestOffer.discountPct}%</span>
                    )}
                </div>
                <div className="mt-4 flex flex-wrap items-center gap-2">
                    <PlatformPill platform={bestOffer.platformName} />
                    {bestOffer.promotionLabel && <Badge variant="soft">{bestOffer.promotionLabel}</Badge>}
                </div>
                <div className="mt-5 rounded-[22px] border border-white/60 dark:border-stone-700/40 bg-white/70 dark:bg-stone-800/40 px-4 py-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)] backdrop-blur-sm">
                    <p className="text-[10px] uppercase tracking-[0.12em] text-stone-400">Nơi bán tốt nhất</p>
                    <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-sm font-medium text-stone-900 dark:text-stone-100">{bestOffer.platformName}</p>
                        <a href={buyUrl} target="_blank" rel="noreferrer"
                            ...
                            Đến cửa hàng <MoveUpRight size={15} />
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
                <a href={buyUrl} target="_blank" rel="noreferrer"
                    ...
                    Mua tại {bestOffer.platformName} <MoveUpRight size={16} />
                </a>
                ...
                <button type="button" onClick={handleWishlistClick} disabled={removing}
